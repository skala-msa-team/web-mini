...
const CLIENT_ERROR_MESSAGE = Object.freeze({
  ...
  [CLIENT_ERROR_CODE.REQUEST_TIMEOUT]: '요청 시간이 초과되었습니다. 잠시 후 다시 시도해 주세요.',
...
export function normalizeApiError(error) {
  ...
  if (error?.response) {
    ...
    return new ApiError({
      ...
      message: responseBody?.message || CLIENT_ERROR_MESSAGE[CLIENT_ERROR_CODE.HTTP_ERROR],
  ...
  return new ApiError({
    message: error?.message || CLIENT_ERROR_MESSAGE[CLIENT_ERROR_CODE.UNKNOWN_ERROR],
    cause: error,
  })
}
