...
- Require at least one approving review before merging a PR into `main`, `dev`, `frontend`, or `backend`.
...
- `main`, `dev`, `frontend`, and `backend` are protected branches. Do not force-push to them, delete them, or merge work branches into `main` directly.
